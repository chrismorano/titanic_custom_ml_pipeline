...
class ETLPipeline:
    ...
    def __init__(self, features, target, numerical_to_float, save_flag=True, save_location=None):
    ...
    def extract_cabin_letter(self, df):
        ''' '''
        df = df.copy()
        df['cabin'] = df['cabin'].str.replace(r'\d+', '')
        return df
    ...
    def transform(self, data):
        ...
        if self.save_flag:
            data.to_csv(self.save_location, index=False)
